 ...
 *      Author: Hermann
 *
 *      Adapted for RODOS from Arduino Library by:
...
void VL6180x::getIdentification(struct VL6180xIdentification *temp) {
	...
	temp->idModelRevMajor = VL6180x_getRegister(
			VL6180X_IDENTIFICATION_MODEL_REV_MAJOR);
	temp->idModelRevMinor = VL6180x_getRegister(
			VL6180X_IDENTIFICATION_MODEL_REV_MINOR);
	temp->idModuleRevMajor = VL6180x_getRegister(
			VL6180X_IDENTIFICATION_MODULE_REV_MAJOR);
	temp->idModuleRevMinor = VL6180x_getRegister(
			VL6180X_IDENTIFICATION_MODULE_REV_MINOR);
...
float VL6180x::getAmbientLight(vl6180x_als_gain VL6180X_ALS_GAIN) {
	...
	VL6180x_setRegister(VL6180X_SYSALS_ANALOGUE_GAIN,
			(0x40 | VL6180X_ALS_GAIN)); // Set the ALS gain

	//Start ALS Measurement
	VL6180x_setRegister(VL6180X_SYSALS_START, 0x01);

	delay(100 * MILLISECONDS); //give it time...
	...
	unsigned int alsIntegrationPeriodRaw = VL6180x_getRegister16bit(
...
void VL6180x::setTransmitBuffer(uint16_t val) {
	txBuf[0] = (val >> 8) & 0xFF;
	txBuf[1] = (val) & 0xFF;
}
...
{
	setTransmitBuffer(registerAddr);

	sem->enter();
	i2c->writeRead(_i2caddress, txBuf, 2, rxBuf, 1);
	sem->leave();
...
uint16_t VL6180x::VL6180x_getRegister16bit(uint16_t registerAddr)
{
	...
	sem->enter();
	i2c->writeRead(_i2caddress, txBuf, 2, rxBuf, 2);
	sem->leave();
